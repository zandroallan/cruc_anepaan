...
function cargar_socios_legales(id_tramite) {
    let url =
        project_name + "/mis-tramites/listas/socios-legales/" + id_tramite;
        ...
        function (data, textStatus) {
            ...
            $.each(data, function (i, valor) {
                body +=
                    ...
                    '<a onclick="modal_socio_legal(' +
                    valor.id +
                    ');" class="btn btn-icon btn-outline-primary btn-circle btn-sm mr-2"><i class="fa fa-pen"></i></a>' +
                    '<a onclick="eliminar_socio_legal(' +
                    valor.id +
                    ');" class="btn btn-icon btn-outline-danger btn-circle btn-sm mr-2"><i class="fa fa-trash"></i></a>' +
                    "</td>" +
                    "</tr>";
                j++;
                if (!status) {
    ...
    $.get(
        ...
        function (data, textStatus) {
            ...
            $.each(data, function (key, valor) {
                if (key == "id_estado_particular") id_estado = valor;

                if (key == "id_municipio_particular") id_municipio = valor;
                ...
                if (
                    ...
                    cargar_municipios_edit(
                        id_estado,
                        "dlscs_id_municipio_particular",
                        id_municipio
                    );
                    carga_est_mun = true;
                }
    ...
    $.ajax({
        ...
        error: function (json) {
            if (json.status === 422) {
                ...
                alert(
                    "Ha ocurrido un error inesperado, contacte a su administrador."
                );
            }
        },
    });
